...
const build = () => {
    ...
    // A Crusher with default settings on the track.
    ...
    for (const pitch of [60, 64, 67, 72]) { // a chord so the input is genuinely hot
...
describe("minimal crusher level", () => {
    it("a crushed chord has the same level in TS and wasm", async () => {
        const Q = 200
        ...
        require("node:fs").writeFileSync("/tmp/crush-level.txt", `TS rms ${tsRms.toExponential(4)} WASM rms ${wasmRms.toExponential(4)} delta ${(20 * Math.log10(wasmRms / tsRms)).toFixed(2)} dB\n`)
