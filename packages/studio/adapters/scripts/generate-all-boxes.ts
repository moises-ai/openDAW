...
const acceptors = new Map<PointerTypes, Array<Vertex>>()
const indexAcceptors = (vertex: Vertex): void => {
...
const walkFields = (fields: ReadonlyArray<Field>, visit: (field: Field) => void): void => {
    for (const field of fields) {
        if (field.deprecated) {continue} // deprecated fields aren't serialized; never wire to them
        visit(field)
        field.accept({
...
boxes.forEach(box => walkFields(box.fields(), field =>
    field.accept({visitPointerField: pointer => pointers.push(pointer)})))
let wired = 0
